...
var agentExe = Path.Combine(AppContext.BaseDirectory, "Access-DevTool-Agent.exe");
if (!File.Exists(agentExe))
    throw new FileNotFoundException(
        $"Access-DevTool-Agent.exe not found at '{agentExe}'. " +
        "Build the Access-DevTool-Agent project and copy it alongside Access-DevTool-MCP.exe.");

var pipeName = $"Access-DevTool-{Environment.ProcessId}";
